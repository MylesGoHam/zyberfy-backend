...
def require_login():
    ...
    if path in PUBLIC_PATHS:
        return
    # 3) allow analytics for any logged-in user (no subscription wall)
...
@app.route("/dashboard")
def dashboard():
    # logged-in enforced by @before_request
    conn = get_db_connection()
    row  = conn.execute(
...
def proposal():
    if request.method == "POST":
        ...
        prompt = (
            ...
            f"in a {auto['tone']} tone & {auto['style']} style. Notes: {auto['additional_notes'] or 'none'}"
        )
        try:
            resp = openai.ChatCompletion.create(
                model="gpt-3.5-turbo",
        ...
        r = send_proposal_email(to_email=lead_email, subject=subject, content=body, cc_client=False)
...
def analytics():
    ...
    for d in dates:
        pv = conn.execute(
            "SELECT COUNT(*) AS cnt FROM analytics_events WHERE user_id=? AND event_type='pageview' AND date(timestamp)=?", (uid, d)
        ).fetchone()["cnt"]
        gp = conn.execute(
            "SELECT COUNT(*) AS cnt FROM analytics_events WHERE user_id=? AND event_type='generated_proposal' AND date(timestamp)=?", (uid, d)
        ).fetchone()["cnt"]
        sp = conn.execute(
            "SELECT COUNT(*) AS cnt FROM analytics_events WHERE user_id=? AND event_type='sent_proposal' AND date(timestamp)=?", (uid, d)
        ).fetchone()["cnt"]
        pv_data.append(pv)
        gen_data.append(gp)
        sent_data.append(sp)
