...
# ─── Require login ────────────────────────────────────────────────────────────
PUBLIC = {"/", "/login", "/terms", "/ping", "/stripe_webhook"}
@app.before_request
def require_login():
...
@app.route("/analytics")
def analytics():
    # **ONLY** require login now, no subscription check
...
def automation():
    # still gate automation behind subscription if you like,
    # or remove the `plan_status!='pro'` check here too:
...
# ... keep your /proposal, /generate-proposal, /stripe_webhook etc. as before ...
