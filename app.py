...
<div class="max-w-6xl mx-auto p-6">
    ...
    <div class="flex items-center gap-4 mt-4 md:mt-0">
      ...
      <div class="relative">
        ...
        <div id="dropdown-menu" class="hidden absolute right-0 mt-2 w-48 bg-white border rounded shadow-lg z-50">
          ...
          <a href="{{ url_for('lead_proposal', public_id=user['public_id']) }}" class="block px-4 py-2 hover:bg-gray-100">Proposal Page</a>
  ...
  <div class="mb-6 text-center">
   <a href="{{ url_for('client_proposal', public_id=user['public_id']) }}" class="inline-block px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg shadow hover:bg-blue-700 transition">
  + New Proposal
  </a>  
  </div>
  ...
  <!-- Proposal Table -->
<div class="mt-12">
  <h3 class="text-xl font-semibold mb-4">Your Proposals</h3>
  ...
  {% for proposal in proposals %}
    <tr 
     onclick="window.location.href='{{ url_for('client_proposal', public_id=proposal.public_id) }}'"
     class="hover:bg-gray-100 cursor-pointer"
    >
      <td class="px-4 py-2">{{ proposal.lead_name }}</td>
      <td class="px-4 py-2">{{ proposal.lead_email }}</td>
      <td class="px-4 py-2">
      ...
      <td class="px-4 py-2">{{ proposal.timestamp or "—" }}</td>
    </tr>
  {% endfor %}
</tbody>
    </table>
