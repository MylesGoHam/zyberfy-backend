...
# ─── Authentication + plan_status sync ────────────────────────────────────────
# public endpoints that anyone (even logged-out) can hit:
PUBLIC_PATHS = {"/", "/login", "/terms", "/ping", "/stripe_webhook", "/memberships"}

@app.before_request
def require_login_and_sync():
    # skip static files & truly public paths
    ...
    # sync plan_status from DB on every request
    conn = get_db_connection()
    row = conn.execute(
...
@app.route("/dashboard")
def dashboard():
    # guaranteed logged in + plan_status synced
    conn = get_db_connection()
    row  = conn.execute(
...
@app.route("/analytics")
def analytics():
    # **NO plan_status check** here – you can see analytics as soon as you’re logged in
    conn = get_db_connection()
    user = conn.execute(
