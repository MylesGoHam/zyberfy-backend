...
def memberships():
    ...
    if request.method == "POST":
        if not request.form.get("terms"):
            flash("You must agree to Terms of Service", "error")
            return redirect(url_for("memberships"))

        if not SECRET_BUNDLE_PRICE_ID:
            flash("Payment config missing", "error")
            return redirect(url_for("memberships"))
...
def analytics():
    ...
    if session.get("plan_status") != "pro":
        flash("Analytics is Pro only—please subscribe", "error")
        return redirect(url_for("memberships"))
...
def automation():
    if session.get("plan_status") != "pro":
        flash("Automation is Pro only—please subscribe", "error")
        return redirect(url_for("memberships"))
