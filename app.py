...
# Now also allow /analytics through the public list
PUBLIC_PATHS = {
    "/", "/login", "/terms", "/ping",
...
def analytics():
    ...
    pageviews   = conn.execute(
        "SELECT COUNT(*) AS cnt FROM analytics_events WHERE user_id = ?", (uid,)
    ).fetchone()["cnt"] if uid else 0
    ...
    for d in dates:
        pv = conn.execute(
            "SELECT COUNT(*) AS cnt FROM analytics_events WHERE user_id = ? AND date(timestamp) = ?",
            (uid, d)
        ).fetchone()["cnt"] if uid else 0
        gp = conn.execute(
            "SELECT COUNT(*) AS cnt FROM analytics_events WHERE user_id = ? AND date(timestamp) = ?",
