...
def memberships():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # store Stripe Customer ID immediately
...
def automation():
    # still require Pro for automation
...
def generate_proposal():
    ...
    try:
        ...
        fallback = (
            f"Hi there—here’s your {auto['tone']} & {auto['style']} proposal.\n\n"
...
def analytics():
    ...
    pageviews   = conn.execute(
        ...
        "WHERE user_id = ? AND event_type = 'pageview'", (uid,)
    ).fetchone()["cnt"]
    generated   = conn.execute(
        "SELECT COUNT(*) AS cnt FROM analytics_events "
        "WHERE user_id = ? AND event_type = 'generated_proposal'", (uid,)
    ).fetchone()["cnt"]
    conversions = conn.execute(
        "SELECT COUNT(*) AS cnt FROM analytics_events "
        "WHERE user_id = ? AND event_type = 'sent_proposal'", (uid,)
    ).fetchone()["cnt"]
    conversion_rate = round(conversions / generated * 100, 1) if generated else 0
    ...
    # Series data
    pageviews_data, gen_data, sent_data = [], [], []
    for d in dates:
        pv = conn.execute(
            "SELECT COUNT(*) AS cnt FROM analytics_events "
